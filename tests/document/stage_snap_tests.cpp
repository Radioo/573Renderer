...
TEST_CASE("A moved object snaps to a guide on its own axis") {
    const Document::StageOutline moving = Square(1, 100, 100, 200);
    const std::vector<Document::SnapGuide> guides{{.vertical = false, .at = 333},
                                                  {.vertical = true, .at = 1000}};
    const Document::Snapped below =
        Document::SnapMove(moving, {}, guides, kStage, Document::Point{11, 29}, 8);
    CHECK_THAT(below.offset[0], WithinAbs(11, 1e-9));
    CHECK_THAT(below.offset[1], WithinAbs(33, 1e-9));
    CHECK(below.guides == std::vector<Document::SnapGuide>{{.vertical = false, .at = 333}});

    const Document::Snapped across =
        Document::SnapMove(moving, {}, guides, kStage, Document::Point{697, 0}, 8);
    CHECK_THAT(across.offset[0], WithinAbs(700, 1e-9));
    CHECK(across.guides == std::vector<Document::SnapGuide>{{.vertical = true, .at = 1000}});
}
