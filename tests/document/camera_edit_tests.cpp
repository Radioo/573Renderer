...
TEST_CASE("A camera added to a frame lands on that frame with both fields present") {
    ...
    REQUIRE(Document::AddCamera(animation, Document::ClipId{}, 2, 7).has_value());
...
TEST_CASE("A frame holds one camera") {
    ...
    REQUIRE(Document::AddCamera(animation, Document::ClipId{}, 0, 1).has_value());
    CHECK_FALSE(Document::AddCamera(animation, Document::ClipId{}, 0, 2).has_value());
    CHECK_FALSE(Document::AddCamera(animation, Document::ClipId{}, 9, 1).has_value());
    CHECK(animation.root.tags.size() == 1);
}
...
TEST_CASE("Removing a camera leaves the clip consistent") {
    ...
    REQUIRE(Document::AddCamera(animation, Document::ClipId{}, 0, 1).has_value());
    REQUIRE(Document::AddCamera(animation, Document::ClipId{}, 2, 1).has_value());
    REQUIRE(Document::RemoveCamera(animation, Document::ClipId{}, 0).has_value());
    CHECK(FramesAreConsistent(animation.root));
    CHECK_FALSE(Document::CameraTag(animation.root, 0).has_value());
    CHECK(Document::CameraTag(animation.root, 2).has_value());
    CHECK_FALSE(Document::RemoveCamera(animation, Document::ClipId{}, 0).has_value());
}

TEST_CASE("Camera fields read back what was written into them") {
    AfpAnimation::Animation animation = Clip(1);
    REQUIRE(Document::AddCamera(animation, Document::ClipId{}, 0, 3).has_value());
...
TEST_CASE("A camera the editor writes survives the animation format") {
    AfpAnimation::Animation animation = Clip(2);
    REQUIRE(Document::AddCamera(animation, Document::ClipId{}, 1, 2).has_value());
