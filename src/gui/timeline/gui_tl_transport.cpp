...
constexpr float kZoomSliderWDips = 120.0F;

float ZoomSliderW() {
    return Gui::Dpi::S(kZoomSliderWDips);
}

bool TransportButton(const char* label, const char* tip) {
    const bool pressed = ImGui::Button(label, ImVec2(Gui::Dpi::S(32.0F), 0.0F));
...
void DrawToggles(Ctx& ctx) {
    ...
    ImGui::SameLine(0.0F, Gui::Dpi::S(8.0F));
...
float TailWidth(const Ctx& ctx) {
    ...
    float width = ImGui::CalcTextSize("[").x + pad + Gui::Dpi::S(3.0F) +
                  ImGui::CalcTextSize("]").x + pad + Gui::Dpi::S(12.0F);
    width += ZoomSliderW();
    width += Gui::Dpi::S(4.0F) + ImGui::CalcTextSize("fit").x + pad;
    width += Gui::Dpi::S(12.0F) + ImGui::CalcTextSize("undo 000").x;
    if (ErrorCount() > 0) {
        width += Gui::Dpi::S(12.0F) + ImGui::CalcTextSize("000 error").x + pad;
    }
    width += Gui::Dpi::S(12.0F) + ImGui::CalcTextSize(DocumentBadge(ctx).c_str()).x + pad;
    return width;
}
