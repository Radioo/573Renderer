...
Dear ImGui Test Engine against a null backend (docs/gui_tests.md). 192 cases,
about 8 seconds - the bulk of the `ctest -L ci` wall time, and still cheap
enough that the local gate stays under ten seconds.

33 of those cases DO want a D3D9 device or a real window: eleven that load a
synthetic 3D scene or 2D package through a WARP device, and twenty-two that
drive the real Win32 window (`window_tests.cpp`, `render_window_tests.cpp`). They follow `pixel_golden_tests` and SKIP (ctest reports
"Skipped") when no device can be created, so a CI leg without one stays green
and simply covers less. Nothing about them needs game data - the assets are
generated byte by byte at test time (docs/gui_tests.md 12).

`gui_tests` is deliberately absent from the ASan job's target list, which stays restricted to
