...
Dear ImGui Test Engine against a null backend (docs/gui_tests.md). 139 cases,
about 8 seconds - the bulk of the `ctest -L ci` wall time, and still cheap
enough that the local gate stays under ten seconds.

17 of those cases DO want a D3D9 device: the seven that load a synthetic 3D
scene or 2D package through a WARP device, and the ten that drive the real
Win32 window. They follow `pixel_golden_tests` and SKIP (ctest reports
