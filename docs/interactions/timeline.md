...
|---|---|---|---|---|---|
| 1 | `label-combo-open` | "go to label..." combo box (preview shows the active label when set) | combo-select | yes | 2 |
| 2 | `track-drag-scrub` | Timeline track drag-scrub | drag | yes | 7 |
...
| 5 | `label-combo-tooltip` | Tooltip on the label combo | hover | yes | 2 |
| 6 | `no-scene-placeholder` | "Load an IFS to control playback." placeholder text | hover | - | **none** |
| 7 | `track-drag-tooltip` | Tooltip on the timeline track body | hover | yes | 7 |
| 8 | `track-label-tick-tooltip` | Label tick mark on the track (hit-tested region inside the custom-drawn track) | hover | yes | 7 |
...
| 25 | `track-click-seek` | Timeline track (custom-drawn InvisibleButton scrub bar) | left-click | yes | 7 |
| 26 | `track-inert-when-total-zero` *(audit)* | Timeline track (##tl_track) in the mc_total == 0 state - hit target exists but is fully inert | left-click | - | 7 |
| 27 | `track-label-tick-click` | Label tick mark on the track (click target) | left-click | yes | 7 |
...
| 35 | `timeline-dock-child-region` | Timeline dock container (fixed-height child window) | scroll | - | 5 |
...
- **tests**: `timeline label combo is absent without labels`, `timeline track and label combo explain themselves`
...
- **tests**: `timeline label tick names the label under the pointer`, `timeline label tick on the track jumps to that label`, `timeline scrub seeks through a label tick instead of stalling`, `timeline shows a hint and no transport before a scene loads` (+3 more)
...
- **tests**: `keyboard nav activates a transport button`, `scenario: pause, step, jump to a label, then resume`, `timeline label tick names the label under the pointer`, `timeline track and label combo explain themselves` (+1 more)
